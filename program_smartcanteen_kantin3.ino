...
// const char* serverURL = "http://192.168.1.4/web_penjual/get_UID_3.php";

const char* ssid = "nisazahra";  // Ganti dengan SSID WiFi Anda
const char* password = "nisazzhra";  // Ganti dengan password WiFi Anda
const char* serverURL = "http://192.168.213.98/web_penjual/get_UID_3.php";
...
void loop() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (httpResponseCode > 0) {
      ...
      Serial.println("(Kantin 3) UID kartu yang dikirim ke server: " + uid);
    } else {
      Serial.println("(Kantin 3) Gagal mengirim data ke server");
